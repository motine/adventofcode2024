...
def concat_digits(a, b)
  b_digit_count = Math.log10(b).floor + 1
  a * 10**b_digit_count + b
end
...
class Equation
  ...
  def possible_step(value_so_far, numbers_left)
    ...
    possible_step(value_so_far + num, rest) || possible_step(value_so_far * num, rest) || possible_step(concat_digits(value_so_far, num), rest)
  end
end
